...
public:
	...
	explicit EpollData(int fd) { m_data.fd = fd; }//显式避免从int到void的隐式转换*
...
public:
	int Create(unsigned count) {
		...
		if (m_epoll < 0) return -2;
		return 0;
	}
	//小于0表示错误，等于0表示无事发生，大于0表示成功拿到事件
	...
	int Del(int fd) {
		if (m_epoll == -1)return -1;
		int ret = epoll_ctl(m_epoll, EPOLL_CTL_ADD, fd, NULL);
		if (ret == -1)return -2;
		return 0;
	}
	void Close() {

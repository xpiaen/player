...
class LogInfo {
public:
    LogInfo(const char* file, int line, const char* func, int pid, pthread_t tid, LogLevel level, const char* fmt, ...);
    LogInfo(const char* file, int line, const char* func, int pid, pthread_t tid, LogLevel level);
    LogInfo(const char* file, int line, const char* func, int pid, pthread_t tid, LogLevel level, void* pData, size_t nSize);
    ~LogInfo();
...
private:
    bool bAuto;//默认为false 流式日志，则为true
    Buffer m_buf;
};
...
public:
    ...
    static void Trace(const LogInfo& info) {
        //thread_local每一次线程调用此函数都会创建一个新的client对象，所以每个线程都有自己的日志文件
        ...
        if (client == -1) {
            ...
            printf("%s(%d):[%s]ret=%d errno:%d msg:%s client:%d\n", __FILE__, __LINE__, __FUNCTION__, ret, errno, strerror(errno), (int)client);
            ret = client.Link();
            printf("%s(%d):[%s]ret=%d errno:%d msg:%s client:%d\n", __FILE__, __LINE__, __FUNCTION__, ret, errno, strerror(errno), (int)client);
        }
